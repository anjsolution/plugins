...
def md_file_link(label: Any, path: Any) -> str:
    ...
    실측으로 정한 두 가지 규칙이 있다(Claude Code 터미널·앱 양쪽에서 6+2 조합 확인):

    1. URL 을 `<...>` 로 **감싸지 않는다.** CommonMark 는 허용하지만 앱이 감싼 링크를 열지 못한다.
    2. 괄호만 퍼센트 인코딩한다. 마크다운 링크는 `](...)` 라서 파서가 경로 안의 괄호를 중첩으로
       센다. `(202605940)` 처럼 짝이 맞으면 되감기가 맞아 그대로 열리지만, 짝이 안 맞는 `(` 가
       하나라도 있으면 맨 끝 `)` 를 링크 종료가 아니라 경로의 일부로 먹어 경로가 틀어진다.
       한글·공백·대괄호는 영향이 없어 그대로 둔다 — 읽을 수 있는 편이 낫다.
    """
    url = file_url(path).replace("(", "%28").replace(")", "%29")
