...
공공 발주 공고를 **찾고 → 체결된 계약을 확인하고 → 어떻게 됐는지 보고 → 첨부를 받아 읽는** 플러그인입니다.
...
| 질문 | 도구 |
|---|---|
| "VMS 1년치 공고 검색해줘" | `ebid_search_common.py --keyword VMS` |
| "터널 관련 계약된 거 (수의계약 포함)" | `ebid_search_contract.py --keyword 터널` |
| "누가 담당이고 업체 대표는 누구야, 왜 수의로 됐어" | `ebid_search_contract.py --keyword 터널 --detail` |
...
├── scripts/                 공개 CLI 4개(공고 검색·계약 검색·개찰·첨부) + _ebid/ 내부 패키지 (codes.json 에 코드 매핑)
...
| `--detail` 이 느림 | 건당 요청 1회. 기본 30건 상한 — 기간을 좁히거나 `--detail-limit` 조정 |
...
202602664(개찰) / 202602663(첨부), 계약은 `--keyword 장미원 --from 20260801 --detail` 로 확인합니다.
